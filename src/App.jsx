...
function Header({theme, setTheme}){
  return(
    <div className="header-container">
      ...
      <button className={`theme-btn ${theme==='day'? 'day-bg' : 'night-bg'}`} onClick={() => theme==='day'? setTheme('night') : setTheme('day')}></button>
    </div>
  )
}
...
function Tip({todos}) {
  return (
    <p style={{display: todos.length === 0 ? 'block' : 'none'}} className='tip'>
      Your todos shall be here
    </p>
  );
}
function AdditionalTools({todos, setTodos}) {
  ...
  const handleDeleteChecked = () => {
    ...
    setTodos(newTodos)}
  return (
    <div className={`additional-tools ${todos.length > 0 ? '' : 'opacity-zero'}`}>
...
export default function App() {
  ...
  return (
    <div className={`todo-app ${theme==='day' ? 'day-theme' : 'night-theme'}`}>
